-- Onboarding role save: ensure profile row exists and persist role (bypasses empty .select after UPDATE).
...
begin
  ...
  if v_old is distinct from p_role then
    if v_old = 'customer' and p_role in ('customer', 'guard', 'merchant') then
      update public.profiles set role = p_role, updated_at = now() where id = v_uid;
    elsif auth.role() = 'service_role' or public.is_admin() then
      update public.profiles set role = p_role, updated_at = now() where id = v_uid;
    else
