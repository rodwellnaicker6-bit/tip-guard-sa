...
echo "   [ ] Guard: request payout from dashboard (Transfer API if PAYSTACK_SECRET_KEY + bank details)"
echo "   [ ] Admin: mark Processing → Paid (uses admin_update_payout_status RPC)"
echo "   [ ] Reject path: pending → Reject releases hold"
echo "   [ ] Manual steps: docs/OPERATOR_PAYOUT_PROCEDURES.md · e2e/payout-flow.spec.ts"
echo ""
